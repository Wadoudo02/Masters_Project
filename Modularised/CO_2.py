...
def bounds_of_wilson_coefficients(category_bounds):
    ...
    features_needed = ["deltaR_sel", "HT_sel", "n_jets_sel", "delta_phi_gg_sel"]
    for proc in procs.keys():
        df_proc = dfs[proc]

        # Check for missing features
        if not all(f in df_proc.columns for f in features_needed):
            raise ValueError(f"Missing required features in process {proc}")

        # Prepare input tensor for PyTorch NN
        nn_input = torch.tensor(df_proc[features_needed].values, dtype=torch.float32)
        with torch.no_grad():
            probabilities = loaded_model(nn_input).squeeze().numpy()

        # Store NN probabilities + categories back into df
        df_proc["NN_probabilities"] = probabilities
        df_proc["category"] = pd.cut(
            probabilities,
