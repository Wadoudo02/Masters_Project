...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 15 16:40:28 2025
...
for i, feat in enumerate(features_to_plot):
    ...
    if feat == "pt":
        bins, rng, logscale, xlabel = [50, (0, 1000), False, "$p_T$ [GeV]"]
    else:
...
add_SMEFT_weights(dfs["ttH"], cg=cg, ctg=ctg, name="SMEFT_weight", quadratic=True)

palette = sns.color_palette("Set2", n_colors=2)
cat_colours = dict(zip(["SM", "SMEFT"], palette))
...
for i, feat in enumerate(features_to_plot):
    ...
    x = dfs["ttH"][feat_to_plot]
    w = dfs["ttH"]["true_weight"]
    w_SMEFT = dfs["ttH"]["SMEFT_weight"]

    # If requested, normalise the weights so each category's histogram has area=1
    if plot_fraction and len(w) > 0 and w.sum() != 0:
        w = w / w.sum()
        w_SMEFT = w_SMEFT / w_SMEFT.sum()
...
# NN palette
nn_palette = sns.color_palette("hls", n_colors=len(cats_unique))
cat_colours_nn = dict(zip(cats_unique, nn_palette))

# SMEFT palette
smeft_palette = sns.color_palette("Set2", n_colors=2)
cat_colours_smeft = dict(zip(["SM", "SMEFT"], smeft_palette))

features_to_plot = ["deltaR", "HT", "pt", "delta_phi_gg"]

# Create the 4 x 2 figure
fig, axs = plt.subplots(
    2, 4, 
    figsize=(24, 12),  # adjust as needed
    dpi=500, 
    sharey=False  # if you don't want to share y-axes across each row
)

# ------------------------------------------------------------------
# Top row: NN categorisation
# ------------------------------------------------------------------
for i, feat in enumerate(features_to_plot):

    ax_top = axs[0, i]
    
    # Unpack the feature's binning/drawing settings
    if feat == "pt":
        bins, rng, logscale, xlabel = [50, (0, 1000), False, "$p_T$ [GeV]"]
    else:
        bins, rng, logscale, xlabel = vars_plotting_dict[feat]
    ...
    # For each NN category, plot a separate histogram
    for cat in cats_unique:
        cat_mask = (dfs["ttH"]["category"] == cat)
        x = dfs["ttH"][feat_to_plot][cat_mask]
        w = dfs["ttH"]["plot_weight"][cat_mask]
...
# ------------------------------------------------------------------
# Bottom row: SM vs SMEFT
# ------------------------------------------------------------------
# Example function that was in your code
def add_SMEFT_weights(proc_data, cg, ctg, name="new_weights", quadratic=False):
    proc_data[name] = proc_data['true_weight'] * (1 + proc_data['a_cg'] * cg + proc_data['a_ctgre'] * ctg)
    if quadratic:
        proc_data[name] += (
...
# Make sure you've applied the SMEFT weights:
add_SMEFT_weights(dfs["ttH"], cg=cg, ctg=ctg, name="SMEFT_weight", quadratic=True)
...
for i, feat in enumerate(features_to_plot):
    ...
    x = dfs["ttH"][feat_to_plot]
    w_sm = dfs["ttH"]["true_weight"]
    w_smeft = dfs["ttH"]["SMEFT_weight"]

    if plot_fraction and len(w_sm) > 0 and w_sm.sum() != 0:
        w_sm = w_sm / w_sm.sum()
        w_smeft = w_smeft / w_smeft.sum()
...
# If you want to save to file, e.g.:
#fig.savefig("combined_4x2_plot.pdf", dpi=300, bbox_inches="tight")
